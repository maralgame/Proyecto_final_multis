//This program implements the matrix multiplication algorithm with C and OpenMP parallel computing
#include <stdio.h>
#include <stdlib.h>
#include "cheader.h"
...
void MtxMult(int **mtx_a, int **mtx_b, int **mtx_c, int n, int m, int p) {
  int i, j, k;
  //Open MP parallel for implementation
  //Shared and private variables definition is important for correct execution and merging of results
  #pragma omp parallel for shared(mtx_a, mtx_b, mtx_c, n, m, p) private(j, k)
  for (i = 0; i < n; i++) {
    for (j = 0; j < p; j++) {
      for (k = 0; k < m; k++) {
